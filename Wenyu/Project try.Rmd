...
```{r}
medgef <- medge %>% filter(Destination == c("China", "Italy", "Japan", "Republic of Korea")) %>%
...
net_degree2 <- graph_from_data_frame(d=medgef, vertices=mnodef, directed=T)
...
V(net_degree2)$label <- mnodef$Country
V(net_degree2)$label.cex <- 0.5
V(net_degree2)$label.color <- "black"

# Vertex size
V(net_degree2)$size <- rescale(mnodef$cases, to = c(0,10))
  #mnodef$cases/2000
